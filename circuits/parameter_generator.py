...
def generate_mass_action_parameters(input_csv, output_csv=None, rnap_0=100, ribosome_0=1000):
    """
    ...
    if output_csv is None:
        ...
        output_csv = input_path.parent / f"{input_path.stem}_with_mass_action{input_path.suffix}"
    ...
    star_factor = k_star_act_reg / max(k_star_act, 0.01)  # Ratio of activated to non-activated
    ...
    filtered_new_params = [p for p in new_params if p['Parameter'] not in existing_params]
    ...
    if filtered_new_params:
        ...
        print(f"Added {len(filtered_new_params)} mass action parameters to {output_csv}")
...
def print_parameter_mappings(df, rnap_0=100, ribosome_0=1000):
    ...
    print(f"  k_tx_cat (MA catalytic): {param_dict.get('k_tx_cat', 'N/A')} (= k_tx / RNAP_0)")
    ...
    print(f"  k_tl_cat (MA catalytic): {param_dict.get('k_tl_cat', 'N/A')} (= k_tl / Ribosome_0)")
    ...
    print(f"  k_tx_bind_star (MA enhanced binding): {param_dict.get('k_tx_bind_star', 'N/A')}")
    print(f"  k_tx_cat_star (MA enhanced catalytic): {param_dict.get('k_tx_cat_star', 'N/A')}")
    
    # Toehold system
    print("\nToehold system:")
    print(f"  k_tl_unbound_toehold (original): {param_dict.get('k_tl_unbound_toehold', 'N/A')}")
    print(f"  k_tl_bound_toehold (original): {param_dict.get('k_tl_bound_toehold', 'N/A')}")
    print(f"  k_tl_bind_unbound (MA unbound binding): {param_dict.get('k_tl_bind_unbound', 'N/A')}")
    print(f"  k_tl_cat_unbound (MA unbound catalytic): {param_dict.get('k_tl_cat_unbound', 'N/A')}")
    print(f"  k_tl_bind_bound (MA bound binding): {param_dict.get('k_tl_bind_bound', 'N/A')}")
    print(f"  k_tl_cat_bound (MA bound catalytic): {param_dict.get('k_tl_cat_bound', 'N/A')}")
...
if __name__ == "__main__":
    ...
    if os.path.exists(parameters_file):
        ...
        updated_params = generate_mass_action_parameters(
            parameters_file, 
            rnap_0=rnap_0,
            ribosome_0=ribosome_0
        )
        
        # Print parameter mappings
        print_parameter_mappings(
            updated_params,
            rnap_0=rnap_0,
            ribosome_0=ribosome_0
        )
