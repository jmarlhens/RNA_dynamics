...
class STAR(ReactionComplex):
    def __init__(
        ...
        if kinetics_type == KineticsType.MICHAELIS_MENTEN:
            self.rules = self._setup_michaelis_menten(
                regulated, regulator, sequence_name, model
            )
        elif kinetics_type == KineticsType.MASS_ACTION:
            self.rules = self._setup_mass_action(
                regulated, regulator, sequence_name, model
            )
        else:
    ...
    def _setup_mass_action(self, regulated, regulator, sequence_name, model):
        ...
        self.k_unbind_rnap = self.parameters.get(
            "k_tx_unbind", 0.1
        ...
        rule3 = Rule(
            ...
            regulator(state="full", sense=None) + model.monomers[dna_name](sense=None)
        ...
        rule5 = Rule(
            ...
            + (
                ...
                % model.monomers[dna_name](b=None, sense=1)
            )
            >> model.monomers[polymerase_name](b=2)
            % (
                regulator(state="full", sense=1)
                % model.monomers[dna_name](b=2, sense=1)
            ),
        ...
        Observable(
            f"obs_RNAP_STAR_{dna_name}_complex",
            model.monomers[polymerase_name](b=2)
            % (
                regulator(state="full", sense=1)
                % model.monomers[dna_name](b=2, sense=1)
