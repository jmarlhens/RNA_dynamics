...
# RNA 2 repressing RNA 1
parameters_RNA_1_RNA_2 = {'k_init': 1e-0,
                'k_bind': 1e-0,
                'k_unbind': 1e-5,
                'k_act': 1e-2,
                'k_act_reg': 1e-8,
...
parameters_RNA_2_RNA_1 = {'k_init': 1e-0,
                'k_bind': 1e+1,
                'k_unbind': 1e-5,
                'k_act': 1e-4,
                'k_act_reg': 1e+1,
...
RNA_regulatory_base_parameters(model, RNA_1, RNA_2, parameters_RNA_1_RNA_2)
...
RNA_regulatory_base_module(RNA_1, RNA_2, k_init_RNA_1, k_bind_RNA_1_RNA_2, k_unbind_RNA_1_RNA_2, k_act_RNA_1, k_act_reg_RNA_1_RNA_2, k_stop_RNA_1, k_stop_reg_RNA_1_RNA_2, k_deg_RNA_1)
...
plt.plot(t, y['obs_RNA_1_init'], label='RNA_1_init')
plt.title('RNA_1')
plt.xlabel('Time (s)')
plt.ylabel('Concentration (nM)')
plt.legend(loc=0)
...
plt.plot(t, y['obs_RNA_2_init'], label='RNA_2_init')
plt.title('RNA_2')
plt.xlabel('Time (s)')
plt.ylabel('Concentration (nM)')
plt.legend(loc=0)
...
# Add RNA 3 to the system that will add a delay to the system
# We can have RNA 3 activate RNA 1, and RNA 1 activate RNA 2, and RNA 2 inhibits RNA 3
model = Model()
Parameter('omega', omega_val)  # in L
...
# RNA 3 activating RNA 1
parameters_RNA_1_RNA_3 = {'k_init': 1e-0,
                'k_bind': 1e+1,
                'k_unbind': 1e-5,
                'k_act': 1e-4,
                'k_act_reg': 1e+2,
...
parameters_RNA_3_RNA_2 = {'k_init': 1e-0,
                'k_bind': 1e+1,
                'k_unbind': 1e-5,
                'k_act': 1e+2,
                'k_act_reg': 1e-1,
