...
Monomer("RNA_4", ['terminator', 'r', 'state'], {'state': ['init', 'full', 'partial']})

# Define the parameters for the kinetic rates}
# RNA 1 activating RNA 2
parameters_RNA_2_RNA_1 = {'k_init': 4*1e-1,
                'k_bind': 1e+6,
                'k_unbind': 1e-8,
                'k_act': 1*1e-1,
                'k_act_reg': 1e+10,
...
parameters_RNA_3_RNA_2 = {'k_init': 1*1e-1,
                'k_bind': 1e+1,
                'k_unbind': 1e-8,
                'k_act': 1e+3,
                'k_act_reg': 1*1e-7,
...
# RNA 3 inhibiting RNA 4
parameters_RNA_4_RNA_3 = {'k_init': 1*1e-1,
                'k_bind': 1e+1,
                'k_unbind': 1e-8,
                'k_act': 1e+0,
                'k_act_reg': 1*1e-8,
...
# RNA 4 activating RNA 1
parameters_RNA_1_RNA_4 = {'k_init': 4*1e-1,
                'k_bind': 1e+3,
                'k_unbind': 1e-8,
                'k_act': 8*1e-1,
                'k_act_reg': 1e+7,
...
RNA_regulatory_base_parameters(model, RNA_4, RNA_3, parameters_RNA_4_RNA_3)
RNA_regulatory_base_parameters(model, RNA_1, RNA_4, parameters_RNA_1_RNA_4)
...
RNA_regulatory_base_module(RNA_4, RNA_3, k_init_RNA_4, k_bind_RNA_4_RNA_3, k_unbind_RNA_4_RNA_3, k_act_RNA_4, k_act_reg_RNA_4_RNA_3, k_stop_RNA_4, k_stop_reg_RNA_4_RNA_3, k_deg_RNA_4)
RNA_regulatory_base_module(RNA_1, RNA_4, k_init_RNA_1, k_bind_RNA_1_RNA_4, k_unbind_RNA_1_RNA_4, k_act_RNA_1, k_act_reg_RNA_1_RNA_4, k_stop_RNA_1, k_stop_reg_RNA_1_RNA_4, k_deg_RNA_1)
...
Observable("obs_RNA_4_init", RNA_4(state='init'))
...
Observable("obs_RNA_4_partial", RNA_4(state='partial'))
...
Observable("obs_RNA_4_full", RNA_4(state='full'))

# Define the initial conditions
Parameter('RNA_1_0', 1e-4 * model.parameters['omega'].value)  # Initial concentration of RNA_1
...
Parameter('RNA_4_0', 10e-4 * model.parameters['omega'].value) # Initial concentration of RNA_3
...
model.initial(RNA_4(state='init', r=None, terminator=None), RNA_4_0)
...
plt.plot(t, y['obs_RNA_4_full'], label='RNA_4_full')
