...
confidence_interval = 1.96 * (std_fluorescence / np.sqrt(fluorescence_replicates.shape[1]))
...
for replicate in fluorescence_replicates.columns:
    plt.scatter(gfp_concentration, fluorescence_replicates[replicate], alpha=0.5, label=f'Replicate: {replicate}')

# Plot mean and confidence intervals
plt.errorbar(gfp_concentration, mean_fluorescence, yerr=confidence_interval, fmt='-o',
             color='black', label='Mean ± 95% CI', capsize=5)
...
plt.errorbar(gfp_concentration, mean_fluorescence, yerr=confidence_interval, fmt='o',
             label='Mean ± 95% CI', color='blue', capsize=5)

# Regression line
plt.plot(gfp_concentration, fit_line, color='red', label=f'Linear Fit: y = {slope:.2f}x + {intercept:.2f}')
...
plt.figure(figsize=(10, 6))
plt.scatter(gfp_concentration, variance_fluorescence, label='Observed Variance', color='blue')
plt.plot(gfp_concentration, predicted_variance, label=f'Predicted Variance: factor={factor:.4f}', color='red')
