...
def plot_simulation_results(
    ...
    max_conditions = max(len(data['config'].condition_params) for data in simulation_data.values())
    ...
    plt.suptitle(f'Parameter Set {param_set_idx + 1}\nTotal Log Likelihood: {total_ll:.2f}',
    ...
    for circuit_idx, data in simulation_data.items():
        ...
        plt.figtext(0.05, circuit_center,
        ...
        for condition_idx, condition_name in enumerate(config.condition_params.keys()):
            ...
            condition_ll = results_df.loc[param_set_idx, ('likelihood', config.name, condition_name)]
    ...
    for circuit_idx, (circuit_name, data) in enumerate(simulation_data.items()):
        ...
        for condition_idx, condition_name in enumerate(config.condition_params.keys()):
            ...
            exp_data = config.experimental_data[
                config.experimental_data['condition'] == condition_name]
            ax.scatter(exp_data['time'], exp_data['fluorescence'],
                       color='red', alpha=0.6, s=5, zorder=3,
                       label='Experimental data')
